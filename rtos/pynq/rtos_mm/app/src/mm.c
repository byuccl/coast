...
// task handles
static TaskHandle_t mmTaskHandles[NUM_MM_TASKS];
...
void matrix_multiply(mm_t f_matrix[side][side],
        mm_t s_matrix[side][side], mm_t r_matrix[side][side])
{
	int i = 0;
	int j = 0;
	int k = 0;
...
int checkGolden(mm_t results_matrix[side][side], uint32_t xor_golden) {
	unsigned int xor = 0;
	unsigned int i, j;
...
void vStartMMTasks( void ) {
    ...
    for (taskNum = 1; taskNum < NUM_MM_TASKS; taskNum++) {
        ...
        p->m1 = first_matrix;
        p->m2 = second_matrix;
        p->results = (mm_array_ptr_t)pvPortMalloc(matrixSizeBytes);
        p->golden = xor_golden;
