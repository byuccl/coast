...
#define POLL_Q_TASK_NUM	2
static TaskHandle_t __NO_xMR pollQTasks[POLL_Q_TASK_NUM];

// all the parameters for the tasks (just a queue)
static xQueueHandle allPQparams[POLL_Q_TASK_NUM];


/********************************* Functions **********************************/

void vStartPolledQueueTasks( unsigned portBASE_TYPE uxPriority ) __xMR {

	static xQueueHandle xPolledQueue;
	const unsigned portBASE_TYPE uxQueueSize = 10;

	/* Create the queue used by the producer and consumer. */
	xPolledQueue = xQueueCreate(
			uxQueueSize, ( unsigned portBASE_TYPE ) sizeof( unsigned short ) );

	// save the queues into the global array
	allPQparams[0] = xPolledQueue;
	allPQparams[1] = xPolledQueue;

	/* Spawn the producer and consumer. */
	xTaskCreate(
		vPolledQueueConsumer,		/* task function */
		"QConsNB",					/* task name */
		pollqSTACK_SIZE,			/* stack size */
		( void * ) 0,				/* parameters */
		uxPriority,					/* priority */
		&pollQTasks[0]				/* handle */
	);
...
void vEndPolledQueueTasks() __NO_xMR {
	int i;
	for (i = 0; i < POLL_Q_TASK_NUM; i+=1) {
#ifdef VERBOSE_KILL_TASKS
		xil_printf("Deleting task %d (%s)\r\n", i, pcTaskGetName(pollQTasks[i]));
#endif
		vTaskDelete(pollQTasks[i]);
	}
}
/*-----------------------------------------------------------*/

static void vPolledQueueProducer( void *pvParameters ) __xMR {

	unsigned short __NO_xMR usValue = 0, usLoop;
	xQueueHandle *pxQueue;
	const portTickType xDelay = ( portTickType ) 20 / portTICK_RATE_MS;
	...
	pxQueue = (xQueueHandle*) &(allPQparams[(uint32_t) pvParameters]);
...
static void vPolledQueueConsumer( void *pvParameters ) __xMR {

	unsigned short __NO_xMR usData, usExpectedValue = 0;
	xQueueHandle *pxQueue;
	const portTickType xDelay = ( portTickType ) 20 / portTICK_RATE_MS;
...
void vPollingQueueCountPrint( void ) __xMR {
	xil_printf("QProdNB: %d\r\n", sPollingProducerCount);
	xil_printf("QConsNB: %d\r\n", sPollingConsumerCount);
}

void vPollingQueueCountClear( void ) __xMR {
	sPollingProducerCount = 0;
	sPollingConsumerCount = 0;
	sLastPollingProducerCount = 0;
	sLastPollingConsumerCount = 0;
}
