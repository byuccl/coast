...
static TaskHandle_t __NO_xMR blockQTasks[BLOCK_Q_TASK_NUM];
...
void vStartBlockingQueueTasks( unsigned portBASE_TYPE uxPriority ) __xMR {
...
void vEndBlockedQueueTasks(void) __NO_xMR {
	int i;
...
static void vBlockingQueueProducer( void *pvParameters ) __xMR {

	unsigned short __NO_xMR usValue = MAGIC_QUEUE_VALUE;
...
static void vBlockingQueueConsumer( void *pvParameters ) __xMR {

	unsigned short __NO_xMR usData, usExpectedValue = MAGIC_QUEUE_VALUE;
	xBlockingQueueParameters *pxQueueParameters;
...
portBASE_TYPE xAreBlockingQueuesStillRunning( void ) __xMR {
...
// have to mark these as xMR'd because of the global variable use rules
//  but xil_printf still isn't replicated, so it's fine
void vBlockingQueueCountPrint( void ) __xMR {
	portBASE_TYPE xTasks;

	for (xTasks = 0; xTasks < blckqNUM_TASK_SETS; xTasks++ ) {
		xil_printf("QProdB%d: %d\r\n", xTasks, sBlockingProducerCount[xTasks]);
		xil_printf("QConsB%d: %d\r\n", xTasks+1, sBlockingConsumerCount[xTasks]);
	}

	return;
}

void vBlockingQueueCountClear( void ) __xMR {
	portBASE_TYPE xTasks;
