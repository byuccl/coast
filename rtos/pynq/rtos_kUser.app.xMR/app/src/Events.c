...
the tasks. */
static xTaskHandle __NO_xMR xCreatedTasks[ evtNUM_TASKS ];

/* The single queue onto which the controlling task posts, and the four event
tasks block. */
static xQueueHandle __NO_xMR xQueue;
...
void vStartMultiEventTasks( void ) {
	 ...
	 * This way the global counter array is replicated, and the tasks
	 *  can actually access all of the copies. */
	xTaskCreate(
		prvMultiEventTask,
		"Event0",
		evtSTACK_SIZE,
...
static void prvMultiEventTask( void *pvParameters ) __xMR {

	portBASE_TYPE *pxCounter;
	unsigned portBASE_TYPE __NO_xMR uxDummy;
...
static void prvEventControllerTask( void *pvParameters ) __xMR {

	const char * const pcTaskStartMsg = "Multi event controller task started.\r\n";
	portBASE_TYPE xDummy = 0;
	static portBASE_TYPE __NO_xMR xSendVal = MAGIC_EVENT_VALUE;
...
void vMultiEventTasksCountPrint( void ) __xMR {
	xil_printf("mEvents: %d\r\n", xCheckVariable);
	return;
}

void vMultiEventTasksCountClear( void ) __xMR {
	xCheckVariable = 0;
