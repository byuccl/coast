    ...
    for (MachineFunction::iterator MFI = Fn.begin(), E = Fn.end(); MFI != E;
      ...
      for (MachineBasicBlock::iterator I = MBB.begin(), E = MBB.end();
        ...
        if (I->getOpcode() == MSP430::JMP) {
          ...
          MachineBasicBlock* newMBB = MBB.getNextNode();
          TII->ReverseBranchCondition(Cond);
          BuildMI(MBB, I, dl, TII->get(MSP430::JCC))
        		  .addMBB(newMBB).addOperand(Cond[0]);
            //.addImm(4).addOperand(Cond[0]);

          NewSize = 6;
        }
        // Uncond branch to the real destination.
        //Hacky way to tell if there are extended intructions - see MSP430Subtarget.h
        if(Fn.getSubtarget().enableEarlyIfConversion())
			I = BuildMI(MBB, I, dl, TII->get(MSP430::BAi)).addMBB(Dest);
		else
