...
unsigned char __xMR data[64];
uint32_t __xMR state[8];
uint32_t __xMR bitlen[2];

unsigned char __xMR hashGlbl[32];

__attribute__((noinline))
void __xMR sha256_transform(uint32_t ctx_state[], unsigned char data[]) {
...
__attribute__((noinline))
void __xMR sha256_hash(unsigned char ctx_data[], uint32_t ctx_bitlen[],
