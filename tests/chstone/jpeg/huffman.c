...
const static int extend_mask[20] = {
    0xFFFFFFFE, 0xFFFFFFFC, 0xFFFFFFF8, 0xFFFFFFF0, 0xFFFFFFE0,
    0xFFFFFFC0, 0xFFFFFF80, 0xFFFFFF00, 0xFFFFFE00, 0xFFFFFC00,
    0xFFFFF800, 0xFFFFF000, 0xFFFFE000, 0xFFFFC000, 0xFFFF8000,
    0xFFFF0000, 0xFFFE0000, 0xFFFC0000, 0xFFF80000, 0xFFF00000};
...
static int pgetc() {
    ...
    if ((temp = *CurHuffReadBuf++) == MARKER_MARKER) { /* If MARKER then */
        if ((temp = *CurHuffReadBuf++)) { /* if next is not 0xff, then marker */
...
int buf_getv(int n) {
    ...
    while (p > 0) {
        if (read_position >
            23) { /* If byte buffer contains almost entire word */
...
int huff_make_dhuff_tb(int *p_xhtbl_bits, int *p_dhtbl_maxcode,
                       int *p_dhtbl_mincode, int *p_dhtbl_valptr) {
    ...
    for (p_dhtbl_ml = 1, p = 0, l = 1; l <= 16; l++) {
        ...
        } else {                     /* We use -1 to indicate skipping. */
            p_dhtbl_valptr[l] = p;
 ...
 *
 */
int DecodeHuffman(int *Xhuff_huffval, int Dhuff_ml, int *Dhuff_maxcode,
                  int *Dhuff_mincode, int *Dhuff_valptr) {
    int code, l, p;
...
void DecodeHuffMCU(int *out_buf, int num_cmp) {
    ...
    for (k = 1; k < DCTSIZE2;) { /* JPEG Mistake */
        ...
        if (s) {
            ...
            s--;                                       /* Align s */
            if ((out_buf[k] & bit_set_mask[s]) == 0) { /* Also (1 << s) */
