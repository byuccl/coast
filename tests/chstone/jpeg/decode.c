...
void WriteBlock(int *store, int *p_out_vpos, int *p_out_hpos,
     ...
     * Write block
     */
    WriteOneBlock(store, p_out_buf, p_jinfo_image_width, p_jinfo_image_height,
                  voffs, hoffs);
...
void Write4Blocks(int *store1, int *store2, int *store3, int *store4,
    ...
    voffs = *p_out_vpos * DCTSIZE;
    hoffs = *p_out_hpos * DCTSIZE;
    WriteOneBlock(store1, p_out_buf, p_jinfo_image_width, p_jinfo_image_height,
                  voffs, hoffs);
     ...
     */
    hoffs += DCTSIZE;
    WriteOneBlock(store2, p_out_buf, p_jinfo_image_width, p_jinfo_image_height,
                  voffs, hoffs);
    ...
    voffs += DCTSIZE;
    hoffs -= DCTSIZE;
    WriteOneBlock(store3, p_out_buf, p_jinfo_image_width, p_jinfo_image_height,
                  voffs, hoffs);
     ...
     */
    hoffs += DCTSIZE;
    WriteOneBlock(store4, p_out_buf, p_jinfo_image_width, p_jinfo_image_height,
                  voffs, hoffs);
...
void decode_block(int comp_no, int *out_buf, int *HuffBuff) {
    ...
    p_quant_tbl = &p_jinfo_quant_tbl_quantval[(
        int)p_jinfo_comps_info_quant_tbl_no[comp_no]][DCTSIZE2];
    ...
    *out_data_image_height = p_jinfo_image_height;
    ...
    if (p_jinfo_smp_fact == SF1_1_1) {
        ...
        while (CurrentMCU < p_jinfo_NumMCU) {
            /*
             * Decode Y element
             * Decoding Y, U and V elements should be sequentially conducted for
             * the use of Huffman table
             */
            ...
            for (i = 0; i < RGB_NUM; i++) {
                Write4Blocks(&rgb_buf[0][i][0], &rgb_buf[1][i][0],
                             ...
                             &out_data_comp_vpos[i], &out_data_comp_hpos[i],
