...
int full_pel_vector; /* MPEG-1 only */
{
    ...
    motion_residual =
        (h_r_size != 0 && motion_code != 0) ? Get_Bits(h_r_size) : 0;
    ...
    motion_code = Get_motion_code();
    motion_residual =
        (v_r_size != 0 && motion_code != 0) ? Get_Bits(v_r_size) : 0;
