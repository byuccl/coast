 ...
 */
#include <stdio.h>
// renamed the other file with functions
//  so that it doesn't get compiled separately
#include "softfloat_src.h"
...
float64 dfsin(float64 rad) {
    ...
    do {
        ...
        float64_ge(float64_abs(diff), 0x3ee4f8b588e368f1ULL)); /* 0.00001 */
    return app;
}
...
*/
#define N 36
const float64 test_in[N] = {0x0000000000000000ULL,  /*      0  */
...
const float64 test_out[N] = {0x0000000000000000ULL,  /*  0.000000 */
...
int main() {
    ...
    for (i = 0; i < N; i++) {
        ...
        printf("input=%016llx expected=%016llx output=%016llx\n", test_in[i],
               test_out[i], result);
    }
    printf("Result: %d\n", main_result);
    if (main_result == 36) {
